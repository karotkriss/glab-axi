...
You do not need glab-axi installed globally - invoke it with `npx -y glab-axi@0.6.0 <command>`.
If glab-axi output shows a follow-up command starting with `glab-axi`, run it as `npx -y glab-axi@0.6.0 ...` instead.
...
- Use `api` for anything the dedicated commands do not cover, e.g. `npx -y glab-axi@0.6.0 api projects/{project}/members` - `{project}` addresses the current project.
