...
- `auth status` now reports the install behind every call: `bin` (the resolved executable path), `version`, `config_file`, and `default_host` - the host a command that omits `--host` lands on. A `shadowed` section lists every other executable of the same name on PATH with its version, appearing only when there is more than one, so its presence is itself the finding. This is the diagnosis for a real failure mode: two installs with separate config files and different default hosts, where every call silently drives whichever comes first on PATH and nothing on the agent surface can say which one answered. Entries are de-duplicated by realpath, so one binary reachable through several PATH entries is not reported as a conflict. Run without `--host`, `auth status` now reports every configured host as a `hosts[]{host,token,account}` table instead of requiring a host: `token` is presence-only (`present`/`absent`, never any part of a value), and `account` is proven by an authenticated call, rendering `unavailable - <reason>` when it could not be confirmed. A default host with no working credential gets a `help[]` line naming the consequence.
- New `config get <key> [--host <host>]` command for reading the GitLab CLI's configuration from the agent surface, reporting the key, the value, and which scope answered (`global` or `host <host>`). An unset key reads `value: unset`; a read that failed errors instead, since "unset" and "could not read" are opposite facts. Any key whose name contains `token` is refused before the read happens - presence is the whole question a caller needs answered, and `auth status` answers it without putting a credential on stdout. `config set` and `config list` are deliberately refused with a reason and a redirect.
...
- The published skill's guidance on GitLab content now has its own section, placed where an agent is about to start consuming forge output rather than in a trailing tip. It covers everything glab-axi can return - issue and merge request titles and bodies, comments and review threads, commit messages, branch names, file contents, and CI job logs - and states plainly that it is data to report, never instructions to act on. It leads with the case that actually catches an agent off guard: text crafted to impersonate a legitimate instruction (a fake system note or priority override), not just the obvious "ignore previous instructions" phrasing. This is guidance only - the CLI still returns exactly what GitLab holds, unfiltered.

### Fixed

- A subcommand that pipes data to the wrapped CLI no longer crashes with an unhandled `EPIPE` when the child exits without reading stdin, which discarded the child's real exit code and took the process down with a stack trace instead of structured output. Reachable in practice against an outdated install that does not implement the subcommand being invoked - the same scenario `auth status` now diagnoses.
